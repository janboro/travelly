...
def get_lat_lng(street, number, code, city):
    ...
    r = requests.get(f"{base_url}{urllib.parse.urlencode(parameters)}{'&format=geojson'}")
...
def validate_location(street, number, code, city, user_id):
        location = Location.query.filter_by(street=street,\
                                            number=number,\
                                            code=code,\
                                            city=city,\
            ...
            try:
                ...
                if type(value) == str:
                    ...
                    location = Location(street=street, number=number, code=code,\
                            city=city, lat=lat, lng=lng, user_id=user_id)
