...
class AddLocationForm(FlaskForm):
    street = StringField(
        "Street", validators=[DataRequired()], render_kw={"placeholder": "Street"}
    )
    number = StringField(
        "Number", validators=[DataRequired()], render_kw={"placeholder": "Number"}
    )  # string, because numbers can be with letters ex. '48A'
    code = StringField(
        "Code", validators=[DataRequired()], render_kw={"placeholder": "City Code"}
    )
    city = StringField(
...
class UpdateLocationForm(FlaskForm):
    ...
    number = StringField(
        "Number", validators=[DataRequired()], render_kw={"placeholder": "Number"}
    )  # sting, because numbers can be with letters ex. '48A'
    code = StringField(
        "Code", validators=[DataRequired()], render_kw={"placeholder": "City Code"}
    )
    city = StringField(
