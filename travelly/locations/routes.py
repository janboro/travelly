...
from travelly.locations.utils import get_lat_lng, validate_location, remove_accents, import_csv, save_csv
...
def location():
    ...
    locations = Location.query.filter(Location.user_id==current_user.id)\
                .order_by(Location.street).paginate(page=page, per_page=10)
    ...
    if add_location_form.validate_on_submit():
        ...
        value = validate_location(street=street,\
                                number=number,\
                                code=code,\
                                city=city,\
        ...
        location = Location(street=street, number=number, code=code,\
                            city=city, lat=lat, lng=lng, user_id=current_user.id)
    ...
    if CSV_form.validate_on_submit():
        ...
        return redirect(url_for('locations.location'))
    return render_template('location/location.html', locations=locations, 
                            add_location_form=add_location_form, CSV_form=CSV_form)
...
def update_location(location_id):
    ...
    if form.validate_on_submit():
        ...
        if type(value) == str:
            flash(value, 'danger')
            return redirect(url_for('locations.update_location', location_id=location_id))
        else:
            lat = value[0]
            lng = value[1]
