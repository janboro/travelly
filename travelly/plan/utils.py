...
def create_map(locations):
    map = folium.Map(location=[51.1079, 17.0385], zoom_start=12, 
    ...
    for location in locations:
        if location.in_planner == True:
            ...
            color = 'blue'
        folium.Marker([location.lat, location.lng],
                        popup=(f'{location.street} {location.number},\
                                {location.code}, {location.city}'),
                        tooltip=tooltip, icon=folium.Icon(color=color)).add_to(map)
    ...
    for loc1 in locations:
        ...
        for loc2 in locations:
            if loc2.path_order == iterator:
                ...
                folium.PolyLine([(loc1.lat, loc1.lng),
                                (loc2.lat, loc2.lng)],
