...
def b_n_b_reccurrent(matrix, temp_bound, temp_weight,
                    level, temp_path, visited):
    '''
    ...
    for i in range(N):
        if (matrix[temp_path[level - 1]][i] != 0 and
                                visited[i] == False):
            temp = temp_bound
            temp_weight += matrix[temp_path[level - 1]][i]

            if level == 1:
                temp_bound -= ((first_minimum_edge(matrix, temp_path[level - 1]) +
                                first_minimum_edge(matrix, i)) / 2)
            else:
                temp_bound -= ((second_minimum_edge(matrix, temp_path[level - 1]) +
                                first_minimum_edge(matrix, i)) / 2)
            ...
            if lower_bound < final_solution:
                ...
                b_n_b_reccurrent(matrix, temp_bound, temp_weight,
                                level + 1, temp_path, visited)
...
def TSP_bnb(matrix):
    ...
    for i in range(N):
        temp_bound += (first_minimum_edge(matrix, i) +
                        second_minimum_edge(matrix, i))
    ...
    b_n_b_reccurrent(matrix=matrix,
